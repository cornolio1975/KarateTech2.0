  ...
  const startCamera = useCallback(async (modeOverride?: 'user' | 'environment') => {
    try {
      ...
      const stream = await navigator.mediaDevices.getUserMedia({
        video: { facingMode: activeMode, width: 640, height: 480 }
      });
                  ...
                  {cameraReady && (
                    <>
                      ...
                      <button
                        ...
                        className="absolute top-3 right-3 p-2 bg-black/60 hover:bg-black/90 text-white rounded-full backdrop-blur-md transition shadow-lg cursor-pointer z-10"
                        title="Switch Camera"
                      >
